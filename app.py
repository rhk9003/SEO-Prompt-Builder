...
請根據這些主題，幫我產出一份「關鍵字候選清單」，用來丟進 Google 關鍵字規劃工具，並以表格方式呈現：

| 建議關鍵字 | 類型（核心/長尾/品牌/問題型） | 主要搜尋意圖（資訊/比較/交易） | 說明（為什麼值得丟進規劃工具） |

要求：
1. 產出 至少 20–30 組關鍵字。
2. 使用「真實使用者會搜尋的語句」。
3. 儘量避免完全重複，但允許合理變化（族群、場景、地區、需求）。
4. 對你認為優先必測的關鍵字，在說明欄標示。"""
