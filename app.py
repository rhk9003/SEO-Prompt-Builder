...
st.set_page_config(
    ...
    page_icon="📑",
    layout="wide"  # 使用寬螢幕模式，以便左右對照
)

# --- 樣式調整 (Optional) ---
# 讓 Text Area 高度一致，視覺更整齊
st.markdown("""
<style>
    .stTextArea textarea {
...
with col2_s5:
    ...
    prompt5 = f"""請幫我根據我給的資訊/搜尋意圖，給我這篇文章的標題建議清單
...
with col2_s7:
    st.caption("🚀 複製下方的 Prompt 給 AI：")
    # 這裡的標題沿用 Step 6 的變數，如果 Step 6 沒填，也會顯示佔位符
...
if st.button("🗑️ 清空所有輸入欄位"):
    # 透過 rerun 來重置頁面，因為沒有用 session_state 綁定變數，
    # Streamlit 的預設行為是 rerun 時如果沒有特別保留，input 會被清空 (但在這個簡單結構下可能需要 browser refresh)
    # 更保險的做法是建議使用者按 F5，或者使用 JavaScript，但在純 Python 簡單版中，
    # 這裡僅提供一個重新執行的按鈕，實際清空建議直接刷新網頁。
    st.rerun()
